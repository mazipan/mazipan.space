...
const DEFAULT_FIELDS = ['title', 'date', 'slug', 'author', 'coverImage', 'excerpt', 'tags', 'published', 'featured']
...
async function getPostBySlug (slug, fields = DEFAULT_FIELDS, lang = 'id') {
...
async function getAllPosts (fields = DEFAULT_FIELDS, lang = 'id') {
  ...
  return posts.filter(p => p.published).sort((post1, post2) => (post1.date > post2.date ? '-1' : '1'))
}

async function getPagedPost (fields = DEFAULT_FIELDS, page = 1, lang = 'id') {
  const slugs = lang === 'id' ? getPostSlugs() : getPostSlugsEn()
  ...
  const sortedPost = posts.filter(p => p.published).sort((post1, post2) => (post1.date > post2.date ? '-1' : '1'))
...
async function getFeaturedPost (fields = DEFAULT_FIELDS, lang = 'id') {
  const posts = await getAllPosts(fields, lang)
  const postsFiltered = posts.filter((post) => Boolean(post.featured && post.published))
