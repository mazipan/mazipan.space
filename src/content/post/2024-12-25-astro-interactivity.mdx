...
Astro telah menyediakan ekstra *directive*, jika membutuhkan kode framework yang interaktiv.
...
<script>
  ...
  toggleBtn.addEventListener('click', () => {
    ...
    const prevState = menu.getAttribute('data-state');
    const nextState = prevState === 'open' ? 'close' : 'open';

    menu.setAttribute('data-menu', nextState);
  });
</script>
```

Liar ya 😂? Tapi jadi bernostalgia dengan masa lalu kan?
...
Lihat kode selengkapnya pada [Codesandbox mengirim dan berlangganan Custom Event](https://codesandbox.io/p/devbox/rough-fast-j1qiqf)
...
Silahkan berkreasi sendiri untuk contoh kodenya.
