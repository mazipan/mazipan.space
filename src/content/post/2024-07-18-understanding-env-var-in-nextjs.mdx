...
React.js dasarnya adalah javascript framework yang awalnya "hanya" jalan di peramban, maka secara natural dia tidak bisa membaca env var. Env var hanya bisa di-_supply_ dari kode Node.js (server-side) ke kode React melalui cara-cara yang dimungkinkan. Salah satunya adalah melalui proses *replacement* saat build production asset melalui build tool seperti Webpack atau Vite. Saat proses build tersebut berlangsung, kita bisa menambahakan proses untuk me-*replace* kata-kata `process.env.SOMETHING` dengan sesuatu yang statik sehingga bisa dijalankan di peramban dengan aman.

## Env var di Next.js

Memasuki era SSR di mana kode client dan server yang digunakan sama persis (sama-sama JavaScript), memberikan kemungkinan kita untuk menulis kode yang sama yang bisa jalan di dua lingkungan yang berbeda. Menggunakan Node.js (atau runtime lainnya) saat di server dan melalui renderer bawaan peramban seperti biasa ketika sampai di client. Jadinya, kode React (atau framework lainnya) yang sama, bisa saja di render di lingkungan Node.js ataupun bisa di render di peramban.
...
```bash title=.env frame=terminal
NEXT_PUBLIC_SAMPLE='statik env var'
```
...
// It will be transformed at build time to "Hello statik env statik env var"`.
...
Kamu tau cara lain? Silahkan tambahkan cara lain untuk mengakses runtime env var melalui kolom komentar di bagian bawah artikel ini.
